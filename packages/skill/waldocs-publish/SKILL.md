...
3. **Write step-by-step markdown** that **begins with an `## Environment` block**, then the modular steps:
   ...
   Keep it **modular**: each step is one self-contained action, and following them top-to-bottom must produce a working result. **Every step MUST include at least one runnable fenced code block or shell command** (real commands/code from this repo) — never prose-only. Fill examples with realistic **mock values** (real `package@version`, `0x…` ids, https URLs); never use placeholder tokens (`PKG`, `<your-key>`, `…`) or comment stubs (`// fill this in`). **Redact secrets:** never write a full private key, API key, mnemonic, or user-specific account/wallet id — truncate them to a recognizable form like `0x28bd…508b` or `suiprivkey1qz…3w8` (public constants like `package@version`, well-known contract object ids, and URLs stay full). If you used a syntax/API that replaced a deprecated one, say so in the relevant step (e.g. "use `SuiJsonRpcClient`; the old `SuiClient` was removed in @mysten/sui 2.x").
