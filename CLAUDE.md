...
1. `llm.structureAppDoc(markdown, usesProtocols)` → ordered modular **steps** (preserves the leading `## Environment` SDK-version block as the first step). Each step is also **routed**: tagged with the single most-related protocol slug (`Step.protocol`, or `null` = app-only) — by conceptual ownership, not by which SDK it imports (a Walrus Memory feature → `walrus` even though it calls Sui).
2. Upsert each step into the app's per-commit document and Walrus (app namespace `<author>/<repo>/<commit>`), cache in Postgres, write the app `_toc` summary.
3. For each protocol in `usesProtocols`: stub-upsert the protocol; **`llm.describeProtocol`** refreshes the index-card description (always, even when nothing merged — see LLM specifics). Then **exclusive-routing merge**: only the steps routed to *this* protocol are fed to `llm.mergeProtocolDoc` (a step is merged into ONE protocol's doc, never duplicated across all used protocols); if a protocol got no routed steps the merge is skipped. The merge returns the **desired doc** — existing units kept **verbatim** (reorderable, content frozen) plus any genuinely new units. Then **always** `llm.curateShowcase` → `repo.replaceShowcase`.

**In-place upsert / dedup (key invariant):** each entity has ONE persistent `Document` (apps keyed per commit via `findAppDocument`, protocols reuse `latestDocument`). `publish.ts` `upsertUnits` diffs incoming units against stored ones **by `DocUnit.contentHash` (sha256 of content)**: matched content is **never re-`remember()`ed to Walrus** — only its `ord`/`groupTitle`/`title` may be patched (`updateUnitMeta`); only genuinely new content triggers a `memwal.remember()` + new row. This is what keeps re-publishing the same commit from duplicating rows in Postgres or memories in Walrus (Walrus is append-only — `remember()` has no delete/overwrite, so dedup must happen **before** the write). `_toc` is only re-written when the entity is new or content actually changed. Removed units are NOT deleted (safe against a bad LLM return; Walrus can't delete anyway).
